...
#define BEE_SET_TIME                        "BeeSetTime"
#define TIMER_COUNT_NUM     100
#define FEED_DELAY          10
#define WAKE_TIME          20//蜂鸣器响后的20个时钟周期
#define false               0
#define true                1
...
int g_beeStatus = -1;
int wake_flag=0;
int  total_time=0;//waketime+time
...
{
    printf("now set time : %d second.",time);
    total_time=time+WAKE_TIME;
    osTimerId_t id;
    osStatus_t status;
    // 创建一个周期性的定时器,回调函数是SleepSomeBed
    id = osTimerNew((osTimerFunc_t)SleepSomeBed, osTimerPeriodic, NULL, NULL);
    ...
    float data2=0;
    data1=GetDistance();
    printf("now count = %d , data1 = %f",count,data1);
    //Lth1550Init();//初始化红外
     while(1)
     {
        printf("now count = %d , data1 = %f",count,data1);
        data2=GetDistance();
        float distance_delta=(data2>data1)?data2-data1:data1-data2;
        if(distance_delta>500.0)
        {
            BeeOff();
            break;
        }
...
void ButtonPressProc(uint8_t ext_io_val)
{
    ...
    uint8_t diff;
    int button1_pressed,button2_pressed,button3_pressed;
...
{
    printf("PAYLOAD:%s\r\n", payload);
    //strstr函数代表在一个字符串中查找是否有子串。
    //所以我们开一个函数叫settime，传一个uint进去，里面实现一个定时器，定时器到头了就调用Beeon
    if (strstr(payload, TRAFFIC_LIGHT_CMD_CONTROL_MODE) != NULL) {
        if (strstr(payload, TRAFFIC_LIGHT_RED_ON_PAYLOAD) != NULL) 
           Beeon();
        else if (strstr(payload, TRAFFIC_LIGHT_RED_OFF_PAYLOAD) != NULL)
           BeeOff();
    }
    if(strstr(payload,BEE_SET_TIME)!=NULL)
        settime(findIntInString(payload));
...
void IotPublishSample(void)
{
    ...
    if (g_beeStatus == 0) {
        property.value = "Bee_ON";
    } else {
        property.value = "Bee_OFF";
    }
